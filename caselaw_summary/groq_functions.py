...
class llmResponse:
    ...
    #                     if sent_limit < 1 and len(docs) < 2:
    #                         break
    ...
    #                 if orig_size > len(docs) or self.sentence_limit > sent_limit+2:
    #                     self.logger.warning(f"Number of documents and/or sentences changed due to being too big for file {file.split('.')[0]}: documents={len(docs)}/{orig_size} - sentences={sent_limit+1}/{self.sentence_limit} - tokens={len(self.tokenizer.encode(doc)) + current_length}")
    #                 if len(self.tokenizer.encode(doc)) + current_length > input_model_params[self.user_model]["max_context_length"]:
    #                     sent_limit = self.sentence_limit
    #                     self.logger.error(f"Summary still too big - {len(self.tokenizer.encode(doc)) + current_length} > {input_model_params[self.user_model]['max_context_length']} - {file.split('.')[0]}. Removing first sentence and trying again.")
    #                     while (len(self.tokenizer.encode(doc)) + current_length > input_model_params[self.user_model]["max_context_length"]) and len(docs) > 0:
    #                         sent_limit = self.sentence_limit
    #                         while (len(self.tokenizer.encode(doc)) + current_length > input_model_params[self.user_model]["max_context_length"]) and sent_limit > 1:
    #                             print(docket_idx, len(self.tokenizer.encode(doc)) + current_length, 1, sent_limit)
    #                             doc = "".join(["".join(doc_sentences[1:sent_limit]) for doc_sentences in docs])
    #                             sent_limit -= 1

    #                         if sent_limit < 3 and len(docs) < 2:
    #                             break
    ...
    #                             docs.pop(random_doc_idx)
    #                             docs_idx.pop(random_doc_idx)
    #                     self.logger.warning(f"Number of documents and/or sentences changed due to being too big for file {file.split('.')[0]}: documents={len(docs)}/{orig_size} - sentences={sent_limit+1}/{self.sentence_limit} (starting from second sentence) - tokens={len(self.tokenizer.encode(doc)) + current_length}")
    ...
    def _response_from_llm(self):
        for extract_sum_type in EXTRACT_TYPES:
            ...
            if extract_sum_type not in os.listdir(f"answers/{self.dataset_name}/{self.model}/{self.prompt_type}/"):
                os.mkdir(f"answers/{self.dataset_name}/{self.model}/{self.prompt_type}/{extract_sum_type}/")
